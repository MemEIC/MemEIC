...
class MiniGPT4(Blip2Base):
    ...
    def __init__(
        ...
        self.one_lora = one_lora  # Store one_lora flag
        use_two_lora = not one_lora  # Changed: use parameter instead of hardcoded True
        if use_lora:

            from peft import get_peft_model, LoraConfig, TaskType
            logging.info('Loading LoRA Adapters for LLAMA')
            ...
            if one_lora:  # One LoRA baseline: single adapter for all editing
                ...
                logging.info("One LoRA baseline builded (single adapter)")
                ...
                self.llama_model.load_adapter(
                    os.path.join(adapter_path, 'textual'),
                    adapter_name='textual'
                )
                self.llama_model.load_adapter(
                    os.path.join(adapter_path, 'connector'),
                    adapter_name='connector'
                )
                logging.info('Adapters loaded from ' + adapter_path)
                ...
                logging.info("LORA(vis/text lora + connector) builded")
            logging.info('Loading LLAMA Adapters Done')
        ...
        if samples['image'] is not None:
            ...
            if self.prompt:
                ...
                for i, prompt_len in enumerate(samples['prompts_len']):
                    targets[i, :prompt_len] = -100
            if self.use_lora:
                inputs_embeds = self.llama_model.get_input_embeddings()(to_regress_tokens.input_ids)
            else:

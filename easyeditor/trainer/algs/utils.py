...
def multimodal_tokenize(batch, processor, device, context_templates=None, hparams=None):
    # Initialize lists to store the processed data from each batch entry
    #FIXME: prompt가 정답 포함인지, label이 prompt 포함인지 다시 체크하기
    len_temp = 1
    prompts = [item for item in batch['edit_inner']['text_input']]
    input_images = [item for item in batch['edit_inner']['image']]
    labels = [item for item in batch['edit_inner']['text_input']]
    file_type = 'image'
    loc_prompts = [item for item in batch['loc']['prompt']]
    loc_prompts_labels = [item for item in batch['loc']['answer']]
    ...
    if hasattr(hparams, 'use_chat_template') and hparams.use_chat_template:
        ...
        prompt_ids = processor(prompts, return_tensors="pt", padding=True, truncation=True)["input_ids"]
    ...
    for i, loc_prompt in enumerate(loc_prompts):
        if loc_prompt in prompts[i]:  # subject: Factual Editing
            subject_token = processor.encode(' ' + loc_prompt, add_special_tokens=False)
            subject_token1 = processor.encode(loc_prompt, add_special_tokens=False)
            subject_length = len(subject_token)
            act_mask = torch.zeros_like(tokens['input_ids'][int(i*len_temp):int((i+1)*len_temp)])
            deact_mask = torch.zeros_like(tokens['input_ids'][int(i*len_temp):int((i+1)*len_temp)])
            for j, token in enumerate(tokens['input_ids'][int(i*len_temp):int((i+1)*len_temp)]):
    ...
    if file_type in ["image", "single-image", "multi-image"]:
       ...
       # multimodal_inputs = processor(images=input_images, text=prompts, return_tensors="pt", padding=True).to(device, dtype=torch.float32)
    elif file_type == "video":
        multimodal_inputs = processor(videos=input_images[0], text=prompts, return_tensors="pt", padding=True).to(device, dtype=torch.float32)

...
class WISE(torch.nn.Module):
    def __init__(self, model, config, device):
        ...
        self.tokenizer = getattr(transformers, config.tokenizer_class).from_pretrained(tok_name, trust_remote_code=True)    
...
class WISEMultimodal(WISE):
    def edit(self, multimodal_inputs, cond, act_mask=None, deact_mask=None, detach_history=False):
        
        text_tokens = multimodal_inputs['text_tokens']
        ans_token_len = multimodal_inputs['ans_token_len']
        ...
        self.layer = re.sub(r'\.(\d+)', r'[\1]', self.layer)
        setattr(eval(f"self.model.{self.layer}"), "training", True)
        setattr(eval(f"self.model.{self.layer}"), "editing", True)
        self.get_adapter_layer().set_parameter_tunable()
        if getattr(eval(f"self.model.{self.layer}"), "editing_total_cnt") % self.config.save_freq == 0:
    ...
    def _cal_ft_loss(self, multimodal_inputs, text_tokens, last_prompt_token_loc, ans_token_len):
        ...
        multimodal_inputs = (multimodal_inputs,) if isinstance(multimodal_inputs, dict) else multimodal_inputs
        ...
        shift_labels = text_tokens['labels'][..., 1:].contiguous()
    ...
    def edit_loss_fn(self, config, pred, targ):
